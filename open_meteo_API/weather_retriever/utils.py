...
def get_location_from_name(place_name: str) -> Tuple[float, float]:
    ...
    try:
        ...
        location_model = LocationModel(latitude=location.latitude, longitude=location.longitude)
...
def organize_data(data:Type[WeatherData], place_name:str) -> None:
    """
    ...
    data_dict = {place_name: {date[i]: {'max_temperature': f"{max_temperature[i]} {temperature_units}",
                              'min_temperature': f"{min_temperature[i]} {temperature_units}",
                              'wind_speed': f"{wind_speed[i]} {wind_speed_units}",
                              'precipitation_sum': f"{precipitation_sum[i]} {precipitation_units}",
                              'type': 'measured' if datetime.strptime(date[i], '%Y-%m-%d') <= datetime.now() else 'forecast'}
                    for i in range(len(date))}}

    for place_name, data in data_dict.items():
        for date, data1 in data.items():
            weather_data = WeatherData(city=place_name, date = date, **data1)
            weather_data.save()
