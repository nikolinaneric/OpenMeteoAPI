...
@pytest.mark.parametrize("place_name, expected_output", [
...
def test_organize_data():
    ...
    for i, d in enumerate(saved_data):    
        ...
        assert d.max_temperature == f"{data.daily.temperature_2m_max[i]} {data.daily_units.temperature_2m_max}"
        assert d.min_temperature == f"{data.daily.temperature_2m_min[i]} {data.daily_units.temperature_2m_max}"
        assert d.wind_speed == f"{data.daily.windspeed_10m_max[i]} {data.daily_units.windspeed_10m_max}"
        assert d.precipitation_sum == f"{data.daily.precipitation_sum[i]} {data.daily_units.precipitation_sum}"
