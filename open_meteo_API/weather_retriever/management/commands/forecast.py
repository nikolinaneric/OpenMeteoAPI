...
class ValidateDateAction(argparse.Action):
    ...
    def __call__(self, parser, namespace, values, option_string=None):
        try:
            ...
            msg = f"Invalid date format: '{values}'.\
                  Must be in the format YYYY-MM-DD."
            parser.error(msg)
        if date > datetime.now().date() + timedelta(days=16):
            msg = f"The date: '{values}'\
                  is too far ahead to have a forecast."
            parser.error(msg)
        if date < datetime.now().date() - timedelta(days=106):
            msg = f"The date: '{values}'\
                  is too far away to have a forecast history."
            parser.error(msg)
        else:
...
class Command(BaseCommand):
    ...
    Instantiates the client for OpetMeteoApi and invokes the
    get_weather_data the method that will return the validated
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "place_name",
            type=str,
            help="City name with hyphens if there \
                are more than one word (Novi-Sad)",
        )
        parser.add_argument(
            "start_date",
            type=str,
            help="Start date of data range (YYYY-MM-DD)",
            action=ValidateDateAction,
        )
        parser.add_argument(
            "end_date",
            type=str,
            help="End date of data range (YYYY-MM-DD)",
            action=ValidateDateAction,
        )
    ...
    def handle(self, *args, **options):
        if options["start_date"] > options["end_date"]:
            raise ValueError(
                f"End date must be greater than \
